    ...
    <div className="space-y-6 xl:space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 xl:gap-6">
        ...
        <div className="bg-card border border-border rounded-lg p-5 xl:p-6 kpi-shadow">
          ...
          <p className="text-2xl xl:text-3xl font-bold text-profit mt-1">{fmt(totalReturned)}</p>
        </div>
      </div>
        ...
        <table className="w-full text-sm min-w-[600px]">
          <thead>
            <tr className="border-b border-border bg-muted/50">
              <th className="text-left px-4 xl:px-6 py-3 xl:py-4 font-medium text-muted-foreground">Client</th>
              <th className="text-right px-4 xl:px-6 py-3 xl:py-4 font-medium text-muted-foreground">Invested</th>
              <th className="text-left px-4 xl:px-6 py-3 xl:py-4 font-medium text-muted-foreground">Expected Return</th>
              <th className="text-right px-4 xl:px-6 py-3 xl:py-4 font-medium text-muted-foreground">Actual Return</th>
              <th className="text-left px-4 xl:px-6 py-3 xl:py-4 font-medium text-muted-foreground">Status</th>
            </tr>
          </thead>
          <tbody>
            {clients.map((c) => (
              <tr key={c.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                <td className="px-4 xl:px-6 py-3 xl:py-4 font-medium text-foreground">{c.name}</td>
                <td className="px-4 xl:px-6 py-3 xl:py-4 text-right text-foreground">{fmt(c.invested)}</td>
                <td className="px-4 xl:px-6 py-3 xl:py-4 text-muted-foreground">{c.expectedReturn}</td>
                <td className="px-4 xl:px-6 py-3 xl:py-4 text-right font-medium text-foreground">{c.actualReturn ? fmt(c.actualReturn) : "—"}</td>
